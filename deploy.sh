...
kubectl apply -f k8s
kubectl set image deployments/client-deployment client=menelik7/multi-client:$SHA
kubectl set image deployments/server-deployment server=menelik7/multi-server:$SHA
kubectl set image deployments/worker-deployment worker=menelik7/multi-worker:$SHA